...
public class HexJavaBeginnerGrid extends JPanel{
	...
	public HexJavaBeginnerGrid(JLabel statusBar, int gridDimensionX, int gridDimensionY, int numBombs, StopWatch s, Record r) {
		...
		for (int i = 20; i < (20+NUM_IMAGES); i++){		//Resize the picture
			image[i-20] = new ImageIcon(this.getClass().getResource((i) + ".png")).getImage();
			img[i-20] = createResizedCopy(image[i-20],21,26,true);
		}
	...
	public void paintComponent(Graphics g)
	{
        ...
        for (int i = 0; i < gridDimX; i++) {
            for (int j = 0; j < gridDimY; j++) {
                if((i+j)%2==1){
                	...
                	System.out.println("Image Index  "+_HexCell.getImageIndex());
        ...
        if (uncover == numMines && numFlags==0 && inGame) {
            ...
            switch(gridDimX){
            	...
            	case 24:
            		rec.run(sw.getTime(),3);
            		break;
            	case 36:
            		rec.run(sw.getTime(),4);
            		break;
            	case 60:
            		rec.run(sw.getTime(),5);
            		break;
