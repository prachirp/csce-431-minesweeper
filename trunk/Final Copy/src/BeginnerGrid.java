...
public class BeginnerGrid extends JPanel {
	...
	public BeginnerGrid(JLabel statusBar, int gridDimensionX, int gridDimensionY, int numBombs, StopWatch sw, Record rec) {
		...
		{
			URL pathString = this.getClass().getResource( i + ".png");
			img[i] = new ImageIcon(pathString).getImage();
		}
...
/*stopped here*/	else	// we are somewhere along the left edge
	...
	public void gameOver(boolean winner)
	{
		...
		else
		{
        	...
        	case 9:
        		record.run(s.getTime(),0);
        		break;
        	case 16:
        		record.run(s.getTime(),1);
        		break;
        	case 30:
        		record.run(s.getTime(),2);
        		break;
